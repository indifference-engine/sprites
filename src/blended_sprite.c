    ...
    float *const viewport_blues)
{
  ...
  for (int row = 0; row < visible_rows; row++)
  {
    for (int column = 0; column < visible_columns; column++)
    {
      ...
      viewport_reds[viewport_index] = viewport_reds[viewport_index] * inverse_opacity + red * sprite_reds[sprite_index] * forward_opacity;
      viewport_greens[viewport_index] = viewport_greens[viewport_index] * inverse_opacity + green * sprite_greens[sprite_index] * forward_opacity;
      viewport_blues[viewport_index] = viewport_blues[viewport_index] * inverse_opacity + blue * sprite_blues[sprite_index] * forward_opacity;
